...
public class Simulator {
    ...
    private static int[] copyRangeThenSort(final int[] arr, final int from, final int to) {
    ...
    public long megaMillionsSim() {
        ...
        final var leftWinner = new int[] {3, 29, 36, 50, 53}; //copyRangeThenSort(left, 0, 5);
        ...
        while (!Arrays.equals(leftWinner, leftTry) || rightTry != rightWinner) {
            Stream.of(left, right).forEach(ArrayUtils::shuffle);
            leftTry = copyRangeThenSort(left, 0, 5);
            rightTry = right[0];
            if (tries++ % 100_000_000L == 0) System.out.printf("So far tried: %,d (trying for %s:%d - time %s)%n",
                    tries - (tries > 1 ? 1 : 0), Arrays.toString(leftWinner), rightWinner, sw);
        }

        System.out.printf("Got a winner matching %s:%d (%s:%d) in %,d tries in time %s.%n",
